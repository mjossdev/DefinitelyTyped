...
declare function which(cmd: string, options: which.Options & which.OptionsAll & which.OptionsNoThrow): Promise<string[] | null>;
declare function which(cmd: string, options: which.Options & which.OptionsFirst & which.OptionsNoThrow): Promise<string | null>;
declare function which(cmd: string, options: which.Options & which.OptionsAll & which.OptionsThrow): Promise<string[]>;
declare function which(cmd: string, options?: which.Options & which.OptionsFirst & which.OptionsThrow): Promise<string>;
declare function which(cmd: string, options: which.Options): Promise<string | string[] | null>;

declare namespace which {
    /** Finds all instances of a specified executable in the PATH environment variable */
    function sync(cmd: string, options: Options & OptionsAll & OptionsNoThrow): string[] | null;
    function sync(cmd: string, options: Options & OptionsFirst & OptionsNoThrow): string | null;
    function sync(cmd: string, options: Options & OptionsAll & OptionsThrow): string[];
    function sync(cmd: string, options?: Options & OptionsFirst & OptionsThrow): string;
    function sync(cmd: string, options: Options): string | string[] | null;
